...
use std::sync::Arc;
use svm_ffi;
pub use svm_ffi::{Struct_svm_parameter, C_SVC, NU_SVC};
use util;
use common;
...
pub fn get_kernel_type(k: Kernel) -> i32 {
    match k {
        Kernel::Linear => svm_ffi::LINEAR,
        Kernel::Poly => svm_ffi::POLY,
        Kernel::Rbf => svm_ffi::RBF,
        Kernel::Sigmoid => svm_ffi::SIGMOID,
    }
}
...
pub fn to_kernel(t: i32) -> Kernel {
    match t {
        svm_ffi::LINEAR => Kernel::Linear,
        svm_ffi::POLY => Kernel::Poly,
        svm_ffi::RBF => Kernel::Rbf,
        svm_ffi::SIGMOID => Kernel::Sigmoid,
...
impl SVMParams {
    ...
    fn from_raw(raw: svm_ffi::Struct_svm_parameter) -> SVMParams {
        SVMParams {
    ...
    fn to_raw(&self) -> svm_ffi::Struct_svm_parameter {
        // let iii: *mut f64 = ptr::null_mut();
        // let jjj: *mut i32 = ptr::null_mut();
        svm_ffi::Struct_svm_parameter {
...
struct RawSVM<'a> {
    ...
    support_vectors: &'a Vec<Vec<svm_ffi::Struct_svm_node>>, // the support vectors
    sv_ptrs: Option<Vec<*const svm_ffi::Struct_svm_node>>, // ptrs to svs
    sv_coefs: &'a Vec<Vec<f64>>,
    sv_coef_ptrs: Option<Vec<*const f64>>, // ptrs to coefs
    raw: Option<svm_ffi::Struct_svm_model>,
}
...
pub struct SVMModel {
    ...
    support_vectors: Vec<Vec<svm_ffi::Struct_svm_node>>, // the support vectors
    // sv_ptrs: Option<Vec<*const svm_ffi::Struct_svm_node>>, // ptrs to svs
...
impl SVMModel {
    ...
    unsafe fn extract_coefs(m: *const svm_ffi::Struct_svm_model) -> Vec<Vec<f64>> {
    ...
    unsafe fn extract_svs(m: *const svm_ffi::Struct_svm_model) -> Vec<Vec<svm_ffi::Struct_svm_node>> {
        let l = (*m).l as usize;
        let mut svs: Vec<Vec<svm_ffi::Struct_svm_node>> = Vec::with_capacity(l);
        let sv_ptr_ptrs: *const *const svm_ffi::Struct_svm_node = (*m).SV;
        for i in 0..l {
            // println!("extracting sv {} out of {}", i, l);
            let mut cur_sv: Vec<svm_ffi::Struct_svm_node> = Vec::new();
            let sv_ptr_offset: *const svm_ffi::Struct_svm_node = *(sv_ptr_ptrs.offset(i as isize));
            let mut j = 0;
            loop {
                // TODO I want to be able to free this memory. If I just deref the pointer,
                // will the struct be copied into the vector?
                let sv_node_offset: *const svm_ffi::Struct_svm_node = sv_ptr_offset.offset(j as isize);
                // println!("ptr_offset: {:?}, node_offset: {:?}", sv_ptr_offset, sv_node_offset);
                let sv: svm_ffi::Struct_svm_node = *sv_node_offset;
                let node = svm_ffi::Struct_svm_node {index: sv.index, value: sv.value };
                assert!(sv_node_offset != &node as *const svm_ffi::Struct_svm_node);
                j += 1;
                cur_sv.push(node);
    ...
    #[allow(non_snake_case)]
    pub fn from_raw(m: *const svm_ffi::Struct_svm_model) -> SVMModel {
        unsafe {
            ...
            let rho = slice::from_raw_parts((*m).rho, (nr_class*(nr_class - 1)/2) as usize).to_vec();
            ...
            // let raw = svm_ffi::Struct_svm_model {
            ...
            // TODO free mem
            // svm_ffi::svm_free_and_destroy_model(&mut (m as *mut svm_ffi::Struct_svm_model) as *mut *mut svm_ffi::Struct_svm_model);
            _model
        }
    }
    ...
    fn to_raw<'a>(&'a self) -> RawSVM<'a> {
        ...
        raw_model.sv_coef_ptrs = Some(util::vec_to_ptrs(&self.sv_coefs).vec);
                ...
                // sv_ptrs: None,
                // sv_coef_ptrs: None,
                // raw: svm_ffi::Struct_svm_model::default(),
        raw_model.raw = Some(svm_ffi::Struct_svm_model {
    ...
    pub fn svm_predict(&self, x: &Vec<f64>) -> f64 {
        ...
        // let m: *const svm_ffi::Struct_svm_model = &self.to_raw() as *const svm_ffi::Struct_svm_model;
        self.predict_sparse(&sparse_x)
    }

    // #[allow(dead_code)]
    // pub fn predict_all(&self, xs: &Vec<Arc<Vec<f64>>>) -> Vec<f64> {
    //     let m = &self.to_raw() as *const svm_ffi::Struct_svm_model;
    ...
    fn predict_sparse(&self, sparse_x: &Vec<svm_ffi::Struct_svm_node>) -> f64 {
        let result = unsafe {
            let raw_model = self.to_raw();
            let m = raw_model.raw.as_ref().unwrap() as *const svm_ffi::Struct_svm_model;
            svm_ffi::svm_predict(m, (&sparse_x[..]).as_ptr())
        };
        result
    }
...
pub fn train_svm(prob: SVMProblem,
                 params: svm_ffi:: Struct_svm_parameter) -> SVMModel {
    // let unsafe_prob = prob.to_unsafe_svm_problem();
    let model: SVMModel = unsafe {
        // Warning: 
        let _model = svm_ffi::svm_train(
                &prob.raw as *const svm_ffi::Struct_svm_problem,
                &params as *const svm_ffi::Struct_svm_parameter);
        let wrapped_model = SVMModel::from_raw(_model);
        // TODO: Is this necessary? NO
        // svm_ffi::svm_free_and_destroy_model(_model as *mut *mut svm_ffi::Struct_svm_model);
        wrapped_model
    };
    model
}
...
pub struct SVMProblem {
    ...
    pub bias: f64,
    pub raw: svm_ffi::Struct_svm_problem,
}
...
impl SVMProblem {
    ...
    pub fn from_training_data(xs: &Vec<Arc<Vec<f64>>>, ys: &Vec<f64>) -> SVMProblem {
        ...
        let labels = ys.clone();
        let raw = svm_ffi::Struct_svm_problem {
            l: ys.len() as i32,
            y: labels.as_ptr(),
